...
struct WeightedSamplePointsResult
{
    std::vector<glm::vec3> pts;
    std::vector<float> radius;
    std::vector<std::vector<int>> nei_lists;
    std::vector<int> border_flags;
...
class WeightedCVT : public GEO::CentroidalVoronoiTesselation
{
public:
    WeightedCVT( GEO::Mesh* mesh, HalfEdgeMesh* mesh2, SDF* sdf );
    void WeightedLloyd( int nb_iter );
    void WritePoints( const std::string& path );
    static std::vector<float> _weights;
    static std::vector<std::mutex> _cell_mutex;
    static std::vector<int> _border_flags;
    static HalfEdgeMesh* _mesh2;
    static HalfEdgeSurfaceTester tester;
    static SDF* _sdf;
...
class WeightedRVTImpl : public GEO::RestrictedVoronoiDiagram {
    /** \brief This class type */
    typedef WeightedRVTImpl<DIM> thisclass;
...
public:
    ...
    WeightedRVTImpl(
        ...
        RVD_( delaunay, mesh ), _mesh2( mesh2 ) {
    ...
    public:
        ...
        {
            glm::vec3 gp0 = glm::vec3( p0[0], p0[1], p0[2] );
            glm::vec3 gp1 = glm::vec3( p1[0], p1[1], p1[2] );
            glm::vec3 gp2 = glm::vec3( p2[0], p2[1], p2[2] );
            glm::vec3 gp3 = glm::vec3( p3[0], p3[1], p3[2] );

            SDF* sdf = WeightedCVT::_sdf;

            float w[4] = {
                -(*sdf)(gp0.x, gp0.y, gp0.z),
                -(*sdf)(gp1.x, gp1.y, gp1.z),
                -(*sdf)(gp2.x, gp2.y, gp2.z),
                -(*sdf)(gp3.x, gp3.y, gp3.z)
            };
            WeightedCVT::_cell_mutex[v].lock();
            if (v_adj == -1 && t_adj == -1)
            {
                WeightedCVT::_border_flags[v] = -1;
            }
            WeightedCVT::_cell_mutex[v].unlock();
            for (int i = 0; i < 4; i++)
            {
                if (w[i] > 0)
                {
                    w[i] = 1.f / std::max( w[i], 0.0001f );
                }
                else
                {
                    w[i] = 0.0001f;
            ...
            glm::vec3 cm = gp0 * w[0] + gp1 * w[1] + gp2 * w[2] + gp3 * w[3];
            cm /= ws;

            float wcm = -(*sdf)(cm.x, cm.y, cm.z);
            if (wcm > 0)
                wcm = 1.f / std::max( wcm, 0.0001f );
            else
            {
                wcm = 0.0001f;
            }
            ...
            for (coord_index_t coord = 0; coord < DIM; coord++) {
                cur_mg_out[coord] += cur_m * wcm * cm[coord];
            }
            locks_.release_spinlock( v );
        }
...
protected:
     ...
     * \brief Destructor
     */
    ~WeightedRVTImpl() override {
        delete_threads();
    }

private:
    /** \brief Forbids construction by copy. */
    WeightedRVTImpl( const thisclass& );
