...
__device__ __forceinline__ float3 operator/( const float3& a, float d )
{
    return make_float3( a.x / d, a.y / d, a.z / d );
}

__device__ __forceinline__ float norm( const float3& a )
{
    return sqrtf( a.x * a.x + a.y * a.y + a.z * a.z );
}

__device__ __forceinline__ float norm2( const float3& a )
{
...
//three seperate atomic add
__device__ __forceinline__ void atomicFlt3Add( float3* dst, const float3& value )
{
...
__device__ __forceinline__ float3 MulMv3x3( float3* m, const float3& v )
{
...
__device__ __forceinline__ double3 operator*( double s, const double3& a )
{
    return make_double3( a.x * s, a.y * s, a.z * s );
}

__device__ __forceinline__ double3 operator/( const double3& a, double d )
{
    return make_double3( a.x / d, a.y / d, a.z / d );
}

__device__ __forceinline__ double norm( const double3& a )
{
    return sqrt( a.x * a.x + a.y * a.y + a.z * a.z );
}

__device__ __forceinline__ double norm2( const double3& a )
{
    return a.x * a.x + a.y * a.y + a.z * a.z;
}

__device__ __forceinline__ double3 cross( const double3& a, const double3& b )
{
    return make_double3(
        a.y * b.z - b.y * a.z,
        a.z * b.x - b.z * a.x,
...
//three seperate atomic add
__device__ __forceinline__ void atomicFlt3Add( double3* dst, const double3& value )
{
    atomicAdd( &(dst->x), value.x );
    atomicAdd( &(dst->y), value.y );
    atomicAdd( &(dst->z), value.z );
...
__device__ __forceinline__ double3 MulMv3x3( double3* m, const double3& v )
{
...
{
    return make_double4( v3.x, v3.y, v3.z, w );
}
